...
# Load the CSV file
file_path = 'data/historical_stock_prices.csv'  # Replace with your correct file path
df = pd.read_csv(file_path)

# Melt the DataFrame to convert multiple close_price columns into one
melted_df = pd.melt(df, 
                    id_vars=['company', 'date'], 
                    value_vars=[col for col in df.columns if col.startswith('close_price')], 
                    var_name='stock_ticker', 
                    value_name='close_price')

# Drop rows where close_price is NaN
melted_df.dropna(subset=['close_price'], inplace=True)

# Remove unnecessary parts of the 'stock_ticker' column to keep only the company name
melted_df['stock_ticker'] = melted_df['stock_ticker'].str.replace('close_price_', '')

# Save the reshaped DataFrame back to a CSV
output_file_path = 'data/historical_stock_prices_single_column.csv'  # Replace with your desired output path
