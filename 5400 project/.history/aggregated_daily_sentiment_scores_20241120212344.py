...
def process_sentiment_data(input_file, output_file):
    ...
    # Apply sentiment mapping with error handling
    def safe_map_sentiment(series):
        return series.map(sentiment_mapping).fillna(0)

    # Convert sentiment labels to numerical scores
    news_df['vader_sentiment_score'] = safe_map_sentiment(news_df['sentiment_label'])
    news_df['finbert_sentiment_score'] = safe_map_sentiment(news_df['finbert_sentiment_label'])
...
process_sentiment_data(
    'data/financial_news_with_vader_and_finbert.csv', 
