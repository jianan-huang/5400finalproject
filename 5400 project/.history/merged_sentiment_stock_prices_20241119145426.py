...
# Load the datasets
converted_file_path = 'data/converted_merged_sentiment_stock_prices.csv'
vader_sentiment_file_path = 'data/financial_news_with_vader_sentiment.csv'

converted_df = pd.read_csv(converted_file_path)
vader_sentiment_df = pd.read_csv(vader_sentiment_file_path)

# Ensure 'date' columns are properly formatted
converted_df['date'] = pd.to_datetime(converted_df['date'])
vader_sentiment_df['date'] = pd.to_datetime(vader_sentiment_df['date'])

# Merge the datasets on 'company' and 'date'
final_merged_df = pd.merge(
    converted_df, 
    vader_sentiment_df[['company', 'date', 'vader_sentiment_score', 'vader_sentiment']], 
    on=['company', 'date'], 
    how='left'
)

# Fill missing vader sentiment scores and labels with 'N/A'
final_merged_df['vader_sentiment_score'].fillna('N/A', inplace=True)
final_merged_df['vader_sentiment'].fillna('N/A', inplace=True)

# Save the final merged dataset
final_output_path = 'data/final_merged_sentiment_stock_prices.csv'
final_merged_df.to_csv(final_output_path, index=False)

print("Final dataset has been merged and saved.")


final_merged_csv_path = 'data/final_merged_sentiment_stock_prices.csv'
final_merged_df = pd.read_csv(final_merged_csv_path)

# Filter the DataFrame to only contain rows without any NaN values in the specified columns
filtered_df = final_merged_df.dropna(subset=['sentiment_score', 'close_price', 'vader_sentiment_score', 'vader_sentiment'])

# Save the filtered dataset
filtered_csv_path = 'data/filtered_final_merged_sentiment_stock_prices.csv'
filtered_df.to_csv(filtered_csv_path, index=False)

print("Filtered data saved to 'data/filtered_final_merged_sentiment_stock_prices.csv'.")
