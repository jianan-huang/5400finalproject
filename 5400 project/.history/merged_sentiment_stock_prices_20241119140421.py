...
# Save the sentiment score data to check the results
...
print(news_df[['cleaned_content', 'vader_sentiment_score']].head())

# Step: Aggregate sentiment score by company and date
# Calculate the average sentiment score per company per day
sentiment_aggregated = news_df.groupby(['company', 'date']).agg({'vader_sentiment_score': 'mean'}).reset_index()

# Classify sentiment as Positive, Negative, or Neutral based on the averaged compound score
sentiment_aggregated['vader_sentiment'] = sentiment_aggregated['vader_sentiment_score'].apply(
...
# Convert 'date' column in merged_df to datetime format
