...
def feature_engineering(stock_data_file, sentiment_data_file, output_file):
    ...
    # 1. Fill missing daily returns with zero (e.g., first day since no previous price)
    combined_df['daily_return'].fillna(0, inplace=True)

    # 2. Backward fill for volatility and moving averages, as it takes time to build up these features
    combined_df['volatility'].fillna(method='bfill', inplace=True)
    combined_df['moving_avg_7'].fillna(method='bfill', inplace=True)
    combined_df['moving_avg_30'].fillna(method='bfill', inplace=True)

    # 3. Forward fill for sentiment scores, as sentiment is often relevant over several consecutive days
